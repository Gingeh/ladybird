...
void NodeWithStyle::apply_style(const CSS::ComputedProperties& computed_style)
{
    ...
    // FIXME: The bounds check is necessary because page().preferred_color_scheme() returns garbage data after startup.
    if ((int)preferred_color_scheme > 2 || preferred_color_scheme == CSS::PreferredColorScheme::Auto) {
